...
/// <summary>
/// チップオブジェクト管理クラス
/// <para>　チップオブジェクトGOのコンポとしてPrefabに実装されるクラス。</para>
/// <para>　自身のチップ画像を引数のチップ種別から判断し、自身のRndererに設定する。</para>
/// </summary>
public class TipObject : TokenObject
{
    /// <summary>
    /// チップ種別
    /// </summary>
    ...
    /// トークン管理クラス
    /// </summary>
    public static TokenObjectMgr<TipObject> parent = null;
    ...
    /// <returns></returns>
    public static TipObject Add(int id, float x, float y, int i, int j, string tipImageName, string tipSpriteName)
    {
        if (parent == null)
        {
            // Resorcesフォルダ内のチップGOのprefab名を指定
            parent = new TokenObjectMgr<TipObject>("TipObject");
        }
